...
if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])) {
	...
	$Tw = $_POST['Tw'];
	$Ta = $_POST['Ta'];
	$queryEdit = mysqli_query($con,"UPDATE pb_kriteria SET tm='$Tm', tw='$Tw', ta='$Ta' WHERE id_pbk=1");
	...
	$query = mysqli_query($con, "SELECT * FROM pb_kriteria WHERE id_pbk=1");
	...
	mysqli_query($con, "UPDATE s_kr SET tm = '$jtm', tw = '$jtw', ta = '$jta', j = '$jml', p = '$prioritas' WHERE id_s = 1");
	echo "<script>
		alert('Data berhasil diubah');
		window.location = '$admin_url' + 'adminweb.php?module=perbandingan';
	</script> ";
	} else {
	echo "<script>
