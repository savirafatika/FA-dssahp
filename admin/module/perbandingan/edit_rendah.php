...
if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])) {
	...
	$S = $_POST['Sedang'];
	$R = $_POST['Rendah'];
	$queryEdit = mysqli_query($con,"UPDATE pb_ta SET tinggi='$T', sedang='$S', rendah='$R' WHERE id_pbta=3");
	...
	$query = mysqli_query($con, "SELECT * FROM pb_ta WHERE id_pbta=3");
	...
	if ($queryEdit) {
		...
		mysqli_query($con, "UPDATE s_ta SET ti = '$ti', se = '$se', re = '$re', j = '$jml', p = '$prioritas' WHERE id_sta = 3");
